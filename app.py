...
def estimate_bmr(profile: Profile) -> tuple[float, str, Optional[float]]:
    """Return BMR, calculation basis, and estimated/known lean body mass."""
    if profile.body_fat is not None:
        lean_mass = profile.weight - profile.body_fat
        # Katch-McArdle: body-composition based BMR.
        return 370 + 21.6 * lean_mass, "인바디 체지방량 기반", lean_mass
...
def calculate_goal(profile: Profile, goal: str) -> dict:
    ...
    # With body-fat data, lean mass makes protein needs more personal.
    protein_base = lean_mass if lean_mass is not None else profile.weight
    protein_per_kg = goal_info["protein"]
    # A higher skeletal-muscle level supports the upper end of protein intake.
    if profile.skeletal_muscle is not None and profile.skeletal_muscle / profile.weight >= 0.42:
        protein_per_kg += 0.15
    protein_g = protein_base * protein_per_kg

    # Fat at 25% of total calories; remaining energy becomes carbohydrate.
...
def render_result(result: dict) -> None:
    with st.container(border=True):
        ...
        st.caption(f"기초대사량 {result['bmr']:,} kcal · 유지 칼로리 {result['maintenance']:,} kcal")


def main() -> None:
    st.set_page_config(page_title="하루 칼로리 & 탄단지 계산기", page_icon="🥗", layout="wide")
    ...
    with st.form("calculator"):
        ...
        with left:
            st.markdown("#### 필수 입력")
            height = st.number_input("키 (cm) *", min_value=100.0, max_value=230.0, value=170.0, step=0.1)
            weight = st.number_input("몸무게 (kg) *", min_value=25.0, max_value=300.0, value=65.0, step=0.1)
            age = st.number_input("나이 *", min_value=14, max_value=100, value=25, step=1)
            activity_name = st.selectbox("활동량 *", list(ACTIVITY_LEVELS))
            sex = st.selectbox("성별 (선택, 입력 시 더 정확함)", ["선택 안 함", "남성", "여성"])
        with right:
            st.markdown("#### 인바디 선택 입력")
            st.info("골격근량과 체지방량을 입력하면 결과가 더 정확해집니다.")
            muscle_enabled = st.checkbox("골격근량 입력")
            skeletal_muscle = (
                st.number_input("골격근량 (kg)", min_value=5.0, max_value=100.0, value=28.0, step=0.1)
                if muscle_enabled else None
            )
            fat_enabled = st.checkbox("체지방량 입력")
            body_fat = (
                st.number_input("체지방량 (kg)", min_value=2.0, max_value=150.0, value=15.0, step=0.1)
                if fat_enabled else None
            )
        submitted = st.form_submit_button("나의 하루 목표 계산하기", type="primary", use_container_width=True)

    if submitted:
        if body_fat is not None and body_fat >= weight:
            st.error("체지방량은 몸무게보다 작게 입력해 주세요.")
            return
        ...
        if body_fat is not None:
            st.caption("✓ 체지방량을 반영한 제지방량 기반 기초대사량을 사용했습니다.")
        elif skeletal_muscle is not None:
            st.caption("✓ 골격근량을 단백질 권장량에 반영했습니다. 체지방량까지 입력하면 칼로리도 더 정교해집니다.")
        else:
        ...
        st.divider()
        st.warning("건강 상태, 임신·수유, 성장기, 질환 또는 전문적인 운동 목표가 있다면 의료·영양 전문가와 상담하세요. 계산 결과는 일반적인 추정치입니다.")
